...
if (method=='all'){
  ...
  edgelist_list = read_edge_list_from_genepairs_intra(paste0(homedir,'/genepairs/'), chrm, quant, method)
  cat("read edgelist\n")
  method_names = c("mean", "median", "max", "min")
  i = 1
  for (el in edgelist_list$el){
    net = output_graph(homedir, genename_folder, el, add_singles, del_neighbors, chrm)
