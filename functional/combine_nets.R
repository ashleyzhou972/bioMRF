...
library(Matrix)
datafolder_intra = paste0(homefolder, "/intra/data/")
datafolder_inter = paste0(homefolder, "/inter/data/")
datafolder_all = paste0(homefolder, "/functional/data/")
...
if (length(attributes(newbig)$x)!= ret1$nnz+ret2$nnz){
  stop("The combined edge count does not agree with intra + inter edge counts!\n")
}
